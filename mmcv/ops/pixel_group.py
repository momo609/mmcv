...
def estimate_confidence(
    label: torch.Tensor, score: torch.Tensor, label_num: int
) -> List[List[float]]:

    point_vector = torch.zeros((label_num, 2), dtype=torch.float32)
    ...
    point_vector.index_add_(0, valid_labels, torch.stack((valid_scores, torch.ones_like(valid_scores)), dim=1))
...
def pixel_group(
    ...
    elif torch.__version__ == 'parrots':
