...
void dynamic_voxelize_forward_npu(const at::Tensor &points, at::Tensor &coors,
    ...
    float grid_x_f = (coors_max_x - coors_min_x) / voxel_x;
    float grid_y_f = (coors_max_y - coors_min_y) / voxel_y;
    float grid_z_f = (coors_max_z - coors_min_z) / voxel_z;
    int32_t grid_x = custom_round(grid_x_f);
    int32_t grid_y = custom_round(grid_y_f);
    int32_t grid_z = custom_round(grid_z_f);
